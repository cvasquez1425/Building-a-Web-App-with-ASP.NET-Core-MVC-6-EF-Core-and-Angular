...
namespace TheWorld.Models
{
    public class WorldContextSeedData
    {
        ...
        public async Task EnsureDeedData()
        {
            if (await _userManager.FindByEmailAsync("cvasquez1425@hotmail.com") == null)
            {
                var user = new WorldUser()
                {
                    UserName    = "cvasquez1425",
                    Email       = "cvasquez1425@hotmail.com"
                };

                await _userManager.CreateAsync(user, "P@ssw0rd!");
            }

            if (!_context.Trips.Any())       // Return a Boolean, are there any trips i the database
            {
                var usTrip = new Trip()
                {
                    DateCreated = DateTime.UtcNow,
                    Name        = "US Trip",
                    UserName    = "cvasquez1425",  // TODO Add UserName
                    Stops = new List<Stop>()
                    {
                        //new Stop() {  Name = "Atlanta, GA", Arrival = new DateTime(2014, 6, 4), Latitude = 33.748995, Longitude = -84.387982, Order = 0 },
                        //new Stop() {  Name = "New York, NY", Arrival = new DateTime(2016, 11, 25), Latitude = 40.712784, Longitude = -74.005941, Order = 1 },
                ...
                _context.Trips.Add(usTrip);   // added to the context, adding the usTrip to the database

                _context.Stops.AddRange(usTrip.Stops);  // it will take all the stops that we've added to the trip object, and add it to the Stops table essentially, inside the db.
                ...
                _context.Trips.Add(drTrip);
                _context.Stops.AddRange(drTrip.Stops);
                // Dominican Republic Trip

                await _context.SaveChangesAsync();          // this will go and actually push that data to that database.
            }
        }        
    }
}
